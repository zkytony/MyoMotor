...
private:
	...
	typedef enum _activeButtonDown
	{LEFT=-1, NONE=0, RIGHT=1}; // -1:left, 0:none, 1:right
...
public:
	...
	//		handle[2]: plot0 
	//		handle[3]: plot1 
	vector<int> plotyy(dvec x, dvec y0, dvec y1);
	vector<int> plotyy(dvec x, dvec y0, dvec y1, float width1, float width2, string style1="b-", string style2="r-");
	vector<int> plotyy(const double* x, const double* y0, const double* y1, const int n);
	vector<int> plotyy(const double* x, const double* y0, const double* y1, const int n, const float width0, const float width1, const string style0, const string style1);
	...
	int MatPlot::slider(dvec x, dvec &initialized_update_buffer,float width);
	int MatPlot::slider(dvec x, dvec &initialized_update_buffer,float width, vector<string> sliderlabel);
	int MatPlot::slider(dvec x, dvec &initialized_update_buffer, dvec lower_limit, dvec upper_limit, float width);
	int MatPlot::slider(dvec x, dvec &initialized_update_buffer, dvec lower_limit, dvec upper_limit, float width, vector<string> sliderLabel);
...
void Graphics_init(int argc, char* argv[], const int win_Xpos, 
				   ...
				   bool* monitor_graphics_exit, char* win_Title );
